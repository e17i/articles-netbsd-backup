...
If, for example, [[https://netbsd.gw.com/cgi-bin/man-cgi?rmt][=rmt(8)=]] is not available, your next option would be to pipe to [[https://netbsd.gw.com/cgi-bin/man-cgi?dd][=dd(1)=]]
...
least one case where the ~afpfsd~ crashed while dumping. This
...
Take caution as this is not yet well enough tested---just use it as a simple example.
