...
** notes
   ...
   - the dump frequency in days can be defined in fstab's fifth entry
     when using snapshots, the snapshot devices are not listed in
     fstab, so this mechanism isn't working. Automating the backup in
     a crontab and defining own dump entries with own frequencies can
     mitigate this.
...
and looking at the contents with interactive restore everything looks
good. Having set up this kind of backups gives a good feeling---now
lets make sure continuously it is not too early..
