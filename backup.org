...
~RCMD_CMD~ to ~ssh~ and address your device by option ~-f
user@host:file~.

If ~rmt~ is not available, your next option would be to pipe to [[https://netbsd.gw.com/cgi-bin/man-cgi?dd][dd(1)]]
using ~ssh~.
#+BEGIN_SRC shell
...
=dump= logs the time, device and level of each dump into
...
damaged system. So, when in case of emergency, use [[https://netbsd.gw.com/cgi-bin/man-cgi?restore][restore(8)]] to apply
...
~newfs~ before. Afterwards, ~mount~ and ~cd~ into the new file system,
...
applied, ~restore~ needs to pass information between the different runs. So it
...
* Some notes on using dump and restore
...
a file system should be backed up. When doing this, =dump= is always
...
Finally, here is my =bin/backup.sh=. When sourced, provides some commands
...
As a side note, while experimenting with =dump= and =restore=, I found
the last dump made on my NeXTStep System some decades ago. And,
believe it or not, the =restore= command on 2020's NetBSD is still
